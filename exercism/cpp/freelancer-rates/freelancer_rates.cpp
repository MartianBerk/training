...
double daily_rate(double hourly_rate) {
    ...
    // rate
    return hourly_rate * HOURS_IN_DAYS;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    return std::ceil(apply_discount(daily_rate(hourly_rate) * DAYS_IN_MONTH, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    return std::floor(budget / (monthly_rate(hourly_rate, discount) / DAYS_IN_MONTH));
}